...
import { Users, Plus, ArrowLeft, Trash2, Pencil, Shield } from 'lucide-react';
...
export default function UsuariosPage() {
  ...
  function abrirModalEditar(usuario: Usuario) {
    ...
    setCargo(usuario.cargo || 'Usuário');
    setModalAberto(true);
  }
  ...
  async function handleSalvarUsuario(e: React.FormEvent) {
    ...
    try {
      if (editandoId) {
        ...
        // Criar novo usuário no banco e no Auth do Supabase
        ...
        // Inserir registro complementar na tabela de usuários
        ...
        alert('Usuário cadastrado com sucesso! Já é possível realizar o login.');
      }
              ...
              {loading ? (
                ...
                usuarios.map((u) => (
                  <tr key={u.id} className="hover:bg-neutral-800/30 transition-colors">
                    <td className="px-6 py-4 font-medium text-neutral-200">{u.email}</td>
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 bg-neutral-950 border border-neutral-800 text-amber-400 rounded-md text-[10px] flex items-center gap-1 w-fit">
                        <Shield className="w-3 h-3" /> {u.cargo || 'Usuário'}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-md p-6 space-y-4 shadow-2xl">
            <h2 className="text-lg font-bold text-neutral-100 flex items-center gap-2 border-b border-neutral-800 pb-3">
              <Users className="w-5 h-5 text-amber-400" /> {editandoId ? 'Editar Usuário' : 'Cadastrar Novo Usuário'}
            </h2>
            ...
            <form onSubmit={handleSalvarUsuario} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold uppercase text-neutral-400 mb-1">Cargo / Função</label>
                <select 
                  value={cargo} 
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-neutral-800">
                ...
                >
                  {salvando ? 'Salvando...' : (editandoId ? 'Atualizar Usuário' : 'Salvar Usuário')}
                </button>
              </div>
            </form>
          </div>
